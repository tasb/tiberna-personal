...
$users = az devops user list --org $org --query "items[].{ name:user.displayName, license: accessLevel.licenseDisplayName, lastAccessedDate:lastAccessedDate }" --top 10000 | ConvertFrom-Json
...
For ($i=0; $i -le $users.Length; $i++) {
    ...
    if ($u.name) {
        $fileContent += "$($u.name), $($u.license), $($u.lastAccessedDate)`r`n";
    }
}
